...
export class ProceduralUtils {
  ...
  static hash(x: number, z: number): number {
    let h = Math.imul(x | 0, this.C1);
    h = (h << 15) | (h >>> 17);
    h = Math.imul(h, this.C2);
    h ^= Math.imul(z | 0, this.C3);
    h = (h << 13) | (h >>> 19);
  ...
  static getTerrainHeight(x: number, z: number, type: 'standard' | 'dunes' | 'islands' | 'volcano' = 'standard'): number {
      if (type === 'dunes') {
          const nx = x * 0.02;
          const nz = z * 0.02;
          let n = this.noise(nx, nz);
          n = 1.0 - Math.abs(n * 2.0 - 1.0);
          return n * n * 12.0 + this.noise(x * 0.15, z * 0.15) * 0.2;
           ...
           let detail = this.warp(x * 0.03, z * 0.03) * 6.0; 
           ...
           if (ringShape > 0.0) {
               let hShelf = this.mix(deepOcean, reefShelf, this.smoothstep(0.0, 0.15, ringShape));
               let hLand = this.mix(beach, ridge, this.smoothstep(0.4, 0.9, ringShape));
               let h = this.mix(hShelf, hLand, this.smoothstep(0.25, 0.4, ringShape));
               return h + detail * this.smoothstep(0.1, 0.3, ringShape);
           } else {
               if (pDist < ringRadius) {
                   return lagoonBed + this.noise(x*0.1, z*0.1) * 2.0;
               } else {
                   return deepOcean;
               }
           }
      ...
      // Standard Highlands
      let h = this.warp(x * 0.015, z * 0.015);
      return (h - 0.5) * 70.0;
  }

  static random(seed: number): number {
    const x = Math.imul(seed | 0, 1597334677) ^ Math.imul((seed >>> 16) | 0, 3812341611);
