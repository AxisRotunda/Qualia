...
@Component({
  ...
  template: `
     <div class="flex flex-col gap-6 select-none pb-4">
        ...
        <div class="flex items-center border-b border-white/10 px-1 relative">
            ...
            <button (click)="activeTab.set('atmos')" 
                    class="tab-btn flex-1" 
                    [class.active]="activeTab() === 'atmos'">
                <span class="material-symbols-outlined text-[16px]">public</span>
                <span>World</span>
                @if (activeTab() === 'atmos') { <div class="active-line"></div> }
            </button>
            
            <button (click)="activeTab.set('light')" 
                    class="tab-btn flex-1" 
            ...
            <button (click)="activeTab.set('phys')" 
                    class="tab-btn flex-1" 
                    [class.active]="activeTab() === 'phys'">
                       ...
                       <div class="grid grid-cols-1 gap-2">
                         @for (opt of atmosphereOptions; track opt.id) {
                           <button (click)="emitAtmosphere(opt.id)" 
                              ...
                              <!-- Color Indicator -->
                              <div class="w-10 h-10 rounded-full flex items-center justify-center shadow-lg transition-transform group-hover:scale-105 shrink-0"
                                   [ngClass]="opt.colorClass">
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-3">
                       <div class="flex justify-between items-end">
                         <label class="section-title">Time of Day</label>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                           <div class="flex justify-between">
                               <span class="text-xs font-medium text-slate-400">Ambient Light</span>
                               <span class="text-xs font-mono text-slate-300">{{ lights().ambient | number:'1.1-1' }}</span>
                           </div>
                           <input type="range" min="0" max="2" step="0.1" 
                                  [value]="lights().ambient" (input)="emitLight('ambient', $event)"
                                  class="modern-range">
                        </div>
                    </div>
                </div>
            ...
            @if (activeTab() === 'phys') {
                <div class="space-y-6">
                    ...
                    <div class="p-4 rounded-xl bg-gradient-to-br from-rose-950/30 to-slate-900 border border-rose-500/20 space-y-4 shadow-lg">
                        ...
                        <div class="flex justify-between text-[9px] text-rose-500/50 uppercase font-bold tracking-wider">
                            <span>Heavy (-20)</span>
                            <span>Zero (0)</span>
                        </div>
                    </div>
                    ...
                    <div class="p-4 rounded-xl bg-gradient-to-br from-emerald-950/30 to-slate-900 border border-emerald-500/20 space-y-4 shadow-lg">
                        ...
                        <div class="flex justify-between text-[9px] text-emerald-500/50 uppercase font-bold tracking-wider">
                            <span>Slow Mo (0.1)</span>
                            <span>Fast (2.0)</span>
                        </div>
                    </div>
  ...
  styles: [`
    ...
    .tab-btn {
        @apply relative pb-3 pt-2 flex flex-col items-center justify-center gap-1 text-[10px] font-bold text-slate-500 hover:text-slate-300 transition-colors uppercase tracking-wider;
    }
    .tab-btn.active {
        @apply text-cyan-400;
    }
    .active-line {
        @apply absolute bottom-0 left-0 w-full h-0.5 bg-cyan-400 shadow-[0_0_10px_cyan];
    ...
    .weather-tile {
        @apply flex flex-col items-center justify-center py-3 rounded-xl bg-slate-800/40 border border-white/5 text-slate-400 hover:bg-slate-800 hover:text-white transition-all active:scale-95;
    }
    .active-weather {
        @apply bg-cyan-600 text-white border-cyan-400 shadow-lg shadow-cyan-900/20;
    }
    ...
    /* Modern Slider */
    .modern-range {
        @apply w-full h-1.5 bg-slate-800 rounded-full appearance-none cursor-pointer border border-white/5 relative;
    }
    .modern-range::-webkit-slider-thumb {
        ...
        margin-top: -7px; /* (1.5 - 16) / 2 approx, fix alignment */
...
export class WorldSettingsPanelComponent {
  ...
  emitTimeScale(e: Event) {
      this.timeScaleChange.emit(parseFloat((e.target as HTMLInputElement).value));
  }
