...
@Component({
          ...
          <div [style.transform]="'translateY(' + scrollOffset() + 'px)'" class="relative z-10 flex flex-col gap-[1px]">
            @for (entity of visibleEntities(); track entity) {
              <div 
                ...
                [class.text-slate-400]="engine.selectedEntity() !== entity && !isHidden(entity)"
                [class.opacity-50]="isHidden(entity)"
              ...
              >
                ...
                <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" 
                     [class.opacity-100]="isLocked(entity) || isHidden(entity)">
                    
                    <button (click)="toggleLock($event, entity)" 
                            class="p-0.5 rounded hover:bg-white/10 text-[14px]"
                            [class.text-amber-500]="isLocked(entity)"
                            [class.text-slate-600]="!isLocked(entity)"
                            title="Toggle Lock">
                        <span class="material-symbols-outlined text-[14px]">{{ isLocked(entity) ? 'lock' : 'lock_open' }}</span>
                    </button>

                    <button (click)="toggleVis($event, entity)" 
                            class="p-0.5 rounded hover:bg-white/10 text-[14px]"
                            [class.text-slate-600]="!isHidden(entity)"
                            [class.text-slate-500]="isHidden(entity)"
                            title="Toggle Visibility">
                        <span class="material-symbols-outlined text-[14px]">{{ isHidden(entity) ? 'visibility_off' : 'visibility' }}</span>
                    </button>
                </div>

                <span class="text-[9px] text-slate-600 font-mono w-6 text-right">{{ entity }}</span>
              </div>
            }
  ...
  `,
  styles: [`
    .bg-cyan-900_40 { background-color: rgba(8, 51, 58, 0.4); }
  `]
})
export class SceneTreeComponent {
  ...
  private readonly ROW_HEIGHT = 33; 
  ...
  onSearchInput(event: Event) {
      ...
      if (target) {
          this.searchQuery.set(NullShield.sanitize(target.value));
      }
  }

  select(e: Entity) {
    if (this.engine.entityMgr.locked.has(e)) return;
    this.engine.interaction.selectEntity(e);
  }

  onContextMenu(e: MouseEvent, entity: Entity) {
      e.preventDefault();
      if (!this.engine.entityMgr.locked.has(entity)) {
          this.select(entity);
      }
  }
  
  toggleLock(e: Event, entity: Entity) {
      e.stopPropagation();
      this.engine.ops.toggleLock(entity);
  }

  toggleVis(e: Event, entity: Entity) {
      e.stopPropagation();
      this.engine.ops.toggleVisibility(entity);
  }

  isLocked(e: Entity) { return this.engine.entityMgr.locked.has(e); }
  isHidden(e: Entity) { return this.engine.entityMgr.hidden.has(e); }
  ...
  private moveSelection(dir: number, list: Entity[], current: Entity | null) {
      ...
      // Skip locked items in keyboard nav
      const startIdx = nextIndex;
      let attempt = 0;
      while (attempt < list.length) {
          const nextEntity = list[nextIndex];
          if (!this.engine.entityMgr.locked.has(nextEntity)) {
              this.select(nextEntity);
              return;
          }
          nextIndex = Math.min(Math.max(nextIndex + dir, 0), list.length - 1);
          if (nextIndex === startIdx) break; // End of list
          attempt++;
      }
  }

  getIcon(entity: Entity): string {
      const name = this.engine.getEntityName(entity).toLowerCase();
