...
import { LSystem, LRule } from '../../../engine/utils/lsystem.utils';
...
export class NatureFloraService {
  ...
  private generateLSystemTree(dna: LSystemDNA, complexity: number, seed: number): THREE.BufferGeometry | null {
    ...
    let pos = new THREE.Vector3(0, flareH - 0.3, 0); 
  ...
  private generatePalm(dna: PalmDNA, seed: number): THREE.BufferGeometry | null {
      ...
      for(let i=0; i<segments; i++) {
          ...
          trunkParts.push(
              Geo.cylinder(r2, r1, segH, 7) // Increased segments for roundness
                 .mapCylinder(r1, segH, 3.0) // 3x vertical repeat for bark texture
                 .toNonIndexed()
      ...
      // Increased segments (2, 6) for smoother arc
      const frondBaseGeo = Geo.plane(0.7, 1.0, 2, 6).toNonIndexed(); 
      
      for(let i=0; i<frondCount; i++) {
          const frondLen = 2.5 + nextRnd();
          
          const frond = frondBaseGeo.clone()
              // Re-create specific geo instance to ensure clean state
              // Easier to just use procedural builder here inside loop
              const f = Geo.plane(0.8, frondLen, 2, 6).toNonIndexed();
              f.mapVertices(v => {
                  const t = (v.y + frondLen/2) / frondLen; // 0 (base) to 1 (tip)
                  
                  // Taper Width: Broad base, sharp tip
                  const widthScale = 1.0 - Math.pow(t, 2.5);
                  v.x *= widthScale;
                  
                  // Gravity Bend (Exponential)
                  const droop = t * t * 2.0; 
                  v.z += droop;
              });
