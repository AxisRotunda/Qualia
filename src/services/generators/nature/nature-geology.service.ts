...
export class NatureGeologyService {
   ...
   */
    private mkRng(seed: number): () => number {
        let s = seed >>> 0;
        return () => {
            s = (Math.imul(s, 0x41C64E6D) + 12345) & 0x7FFFFFFF;
            return s / 2147483648.0; // 0..1
        };
    }

    /**
   * Generates a geyser vent geometry with a base mound and central vent
   * @param seed Random seed for deterministic generation
   * @returns THREE.BufferGeometry representing the geyser vent
   */
    ...
    /**
   * Generates a rock geometry with configurable type and complexity
   * @param type Rock type: 'granite' for jagged rocks, 'sedimentary' for layered rocks
   * @param complexity Level of detail (0.0 to 1.0)
   * @param seed Random seed for deterministic generation
   * @returns THREE.BufferGeometry representing the rock
   */
    generateRock(type: 'granite' | 'sedimentary' = 'granite', complexity: number = 1.0, seed: number = 12345): THREE.BufferGeometry {
        ...
        const mergeThreshold = Math.max(0.0005, 0.001 * (2.0 - complexity));
        builder.mergeVertices(mergeThreshold)
            .computeVertexNormals()
            .scale(0.8, 0.8, 0.8);
    ...
    /**
   * Generates an ice chunk geometry with crystalline structure
   * @param complexity Level of detail and sharpness (0.0 to 1.0)
   * @param seed Random seed for deterministic generation
   * @returns THREE.BufferGeometry representing the ice chunk
   */
    ...
    /**
   * Generates an ice spire geometry with glacial erosion effects
   * @param seed Random seed for deterministic generation
   * @returns THREE.BufferGeometry representing the ice spire
   */
    ...
    /**
   * Generates an ice block geometry with melting and chipping effects
   * @param size The size of the ice block
   * @param seed Random seed for deterministic generation
   * @returns THREE.BufferGeometry representing the ice block
   */
    ...
    /**
   * Generates a cinder block geometry with hollow center
   * @param seed Random seed for deterministic generation
   * @returns THREE.BufferGeometry representing the cinder block, or null if generation fails
   */
    generateCinderBlock(seed: number = 42): THREE.BufferGeometry | null {
        ...
        parts.push(
            ...
            Geo.box(w - 2 * thick, h, d - 2 * thick).toNonIndexed().translate(0, 0, 0).get() // Center Web (Fixed width)
        );

        const mergedGeometry = BufferGeometryUtils.mergeGeometries(parts);
