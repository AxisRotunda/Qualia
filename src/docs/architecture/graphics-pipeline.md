...
## 5. Rendering Flow (Visualized)
The `RenderSystem` orchestrates the final steps to produce an image each frame.

```mermaid
graph TD
    subgraph "RenderSystem.update()"
        A[Interpolate Visuals] --> B[View Culling]
        B --> C[Update Camera]
        C --> D[Update Instanced Meshes]
        D --> E[Update Selection Helper]
        E --> F{"Use Post-Processing?"}
    end
    
    F -- No --> H[Renderer.render()]
    F -- Yes --> G[Composer.render()]

    subgraph "EffectComposer Pipeline"
        direction LR
        G1[RenderPass] --> G2[BloomPass] --> G3[CompositePass] --> G4[OutputPass]
    end
